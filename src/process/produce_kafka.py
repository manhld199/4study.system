...
def get_enrolled_courses(days=None):
    """
    Fetch all courses that all users have enrolled in, grouped by user_id.
    Optionally filter courses enrolled within the past `days` days.
    :param days: Optional, number of days to look back.
    :return: List of users with their courses.
    """
    try:
...
def create_topic_if_not_exists(topic_name, num_partitions=1, replication_factor=1):
    ...
    try:
        ...
        admin_client = KafkaAdminClient(
            bootstrap_servers="localhost:9093",  # Sử dụng hostname của container Kafka
